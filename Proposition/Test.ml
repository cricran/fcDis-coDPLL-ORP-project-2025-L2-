...
(** correct_ex_sat res f : Teste si un témoin de satisfaisabilité est correct :
  - si le témoin (res) est None, teste si la formule est une contradiction avec l'algorithme de Quine
  - sinon, si le témoin (res) est (Some l), teste si la formule est vraie pour l'interprétation obtenue depuis l *)
let correct_ex_sat (_ : (string * bool) list option) (_ : formule) : bool =
  failwith "correct_ex_sat : à faire"

(** correct_all_sat res f : Teste si les témoins de satisfaisabilité sont corrects. 
  Plus précisément, teste si pour chaque sous-liste l2 d'atomes de f,
  l'évaluation de f avec l'interprétation obtenue depuis l2 est équivalente à
  l'existence d'une sous-liste l1 de res telle que pour tout atome (at, b) de l1, b est vrai si (at est dans l1) *)
